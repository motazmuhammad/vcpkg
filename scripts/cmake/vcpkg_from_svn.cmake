function(vcpkg_from_svn)
    ...
    if (NOT PARSE_ARGV_URL)
        message(FATAL_ERROR "vcpkg_from_svn: URL is required")
    endif()

    if (NOT PARSE_ARGV_OUT_SOURCE_PATH)
        message(FATAL_ERROR "vcpkg_from_svn: OUT_SOURCE_PATH is required")
    endif()

    set(SOURCE_PATH ${PARSE_ARGV_OUT_SOURCE_PATH})

    # Default revision or tag to head if none is specified
    if (NOT PARSE_ARGV_REV AND NOT PARSE_ARGV_TAG)
        set(PARSE_ARGV_REV "HEAD")
    endif()

    if (PARSE_ARGV_TAG)
        set(SVN_REV_OPTION "--revision ${PARSE_ARGV_TAG}")
    elseif (PARSE_ARGV_REV)
        set(SVN_REV_OPTION "--revision ${PARSE_ARGV_REV}")
    endif()

    # Check out the repository using SVN
    message(STATUS "Fetching SVN repository from ${PARSE_ARGV_URL} at ${SVN_REV_OPTION}")

    execute_process(
        COMMAND svn checkout ${PARSE_ARGV_URL} ${SOURCE_PATH} ${SVN_REV_OPTION}
        RESULT_VARIABLE SVN_RESULT
    )

    if (NOT SVN_RESULT EQUAL 0)
        message(FATAL_ERROR "Error: failed to fetch repository from SVN")
    endif()

    set(${PARSE_ARGV_OUT_SOURCE_PATH} ${SOURCE_PATH} PARENT_SCOPE)
endfunction()
